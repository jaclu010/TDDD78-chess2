...
public class ChessBoard
{
	...
	logMsg = (selected.getpT().name() + " did " + dmg + " damage to " + board[y][x].getpT().name()) + " at " +
		 GlobalVars.getLetter(x) + (height - 1 - y);
	logger.info(logMsg);
    }

    private void printKill(int y, int x) {
	logMsg = (selected.getpT().name() + " killed " + board[y][x].getpT().name() + " at " + GlobalVars.getLetter(x) +
		  (height - 1 - y));
	logger.info(logMsg);
    }

    private void printPieceMovement(int y, int x) {
	logMsg = (selected.getpT().name() + " from: " + GlobalVars.getLetter(selectedCoords.getX()) +
		  (width - 1 - selectedCoords.getY()) + " -> " + GlobalVars.getLetter(x) + (height - 1 - y));
	logger.info(logMsg);
    }
